...
def project_hrr(bid, pid, lineup_pos, batters, pitchers, league_mix, league_xwoba, hand, p_hand_map,
    ...
    adj_h   = blended["H_pa"]   * quality_mult * hf
    adj_r   = blended["R_pa"]   * quality_mult * hf * team_factor
    adj_rbi = blended["RBI_pa"] * quality_mult * hf * team_factor
    ...
    return {
        ...
        "recent_pa": blended.get("recent_PA", 0), "recent_weight": blended.get("recent_weight", 0.0),
        "recent_h_pa": blended.get("recent_H_pa"), "recent_r_pa": blended.get("recent_R_pa"), "recent_rbi_pa": blended.get("recent_RBI_pa"),
        "blended_h_pa": blended["H_pa"], "blended_r_pa": blended["R_pa"], "blended_rbi_pa": blended["RBI_pa"],
...
def build_email_html(date, rows, stats):
    ...
    return (
        ...
        f"<th style='padding:8px 10px'>Best Line</th>"
        ...
        f"E[HRR] uses each batter's H/R/RBI per PA <em>blended</em> from full-season rates and last-15-day rates "
        f"with shrinkage toward season (recent gets weight = recent_PA / (recent_PA + 100), so 50 PA &asymp; 33% recent, 150 PA &asymp; 60% recent). "
        f"Those blended rates are multiplied by a quality multiplier &times; handedness factor &times; team-offense factor &times; expected plate appearances. "
        f"Quality multiplier = matchup xwOBA / batter's baseline xwOBA (capped 0.75&ndash;1.35). Team factor scales R and RBI by the team&rsquo;s R/G vs the league avg (capped 0.85&ndash;1.15). "
        f"<strong>Recent 15d</strong> column: arrow shows how the player's HRR-per-PA over the last 15 days compares to their season rate (&uarr; &gt;+10%, &darr; &lt;-10%). "
        f"P(O1.5) = P(HRR &ge; 2), P(O2.5) = P(HRR &ge; 3), via Poisson with that expected value. <strong>Edge</strong> = best P(over) &minus; 52.4% (the break-even at standard &minus;110 juice). "
        f"Status: <span style='color:#166534'>&#9989; in</span> = confirmed in lineup, <em>bench</em> = active roster but not in posted lineup, <span style='color:#92400e'>&#9711; TBD</span> = lineup not yet released. "
...
def build_email_text(date, rows, stats):
    ...
    for i, r in enumerate(rows, 1):
        ...
        lines.append(f"          O{r['best_line']}: bet · E[HRR]={r['e_hrr']:.2f} · P(O1.5)={fmt_pct(r['p_over_15'])} · P(O2.5)={fmt_pct(r['p_over_25'])} · Hand×{r['hf']:.2f}{recent_txt}")
        lines.append("")
    return "\n".join(lines)
...
def main():
    ...
    print(f"  fetched savant({len(bat_rows)} batter rows / {len(pit_rows)} pitcher rows), splits, season, teams, recent", flush=True)
    ...
    recent = index_season_hitting(recent_d)
        ...
        def score_side(roster, opp_pp, my_abbr, opp_abbr, lineup_ids, lineup_pos_map, game_label, my_team_id):
            ...
            for pl in roster:
                ...
                rows.append({
                    ...
                    "game": game_label, "in_lineup": in_lineup, **r,
                })
